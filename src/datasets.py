...
def create_dataset(m_total, marker_code = np.array([0,1]).reshape(1,-1), num_code = 100,
    ...
    for i in range(num_code):
      ...
      for j in range(symmm):
        ...
        #for j in range(end_ind):
            ...
            #trainX[i,j,0:gamma-symbol_bit*j] =  0
            #trainX[i,j,gamma-symbol_bit*j:] = -2*y[0, 0:gamma+symbol_bit*j+1] + 1
          #elif gamma < symbol_bit*j and symbol_bit*j < numR-gamma:
            #trainX[i,j,:] = -2*y[0, symbol_bit*j-gamma:symbol_bit*j+gamma+1] + 1
          #elif numR-gamma <= symbol_bit*j and symbol_bit*j < numR + gamma:
            #trainX[i,j,0:numR-symbol_bit*j+gamma] = -2*y[0, symbol_bit*j-gamma:numR] + 1
            #trainX[i,j,numR-symbol_bit*j+gamma:] = 0
      #else:
      #for j in range(end_ind):
        #if (j + 1) % (int(Nc/symbol_bit) + 1) == 0:
          #if symbol_bit*j <= gamma:
